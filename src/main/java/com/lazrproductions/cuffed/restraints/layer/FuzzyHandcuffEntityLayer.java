...
public class FuzzyHandcuffEntityLayer<T extends LivingEntity, M extends HumanoidModel<T>, A extends FuzzyHandcuffsModel<T>>
        extends RenderLayer<T, M> {
    private static final ResourceLocation TEXTURE_LOCATION = new ResourceLocation(CuffedMod.MODID,
            "textures/entity/fuzzy_handcuffs.png");

    private final A model;

    public FuzzyHandcuffEntityLayer(RenderLayerParent<T, M> parent, A model) {
        super(parent);
        this.model = model;
    }
    ...
    public void render(@Nonnull PoseStack stack, @Nonnull MultiBufferSource buffer, int lighting, @Nonnull T entity,
        ...
        if (res.getArmRestraintId().equals(FuzzyHandcuffsRestraint.ID)) {
