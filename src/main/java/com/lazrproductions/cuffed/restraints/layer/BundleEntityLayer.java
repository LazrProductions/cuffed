...
import com.lazrproductions.cuffed.restraints.BundleRestraint;
...
public class BundleEntityLayer<T extends LivingEntity, M extends HumanoidModel<T>, A extends BundleModel<T>>
        extends RenderLayer<T, M> {
    private static final ResourceLocation TEXTURE_LOCATION = new ResourceLocation(CuffedMod.MODID,
            "textures/entity/bundle.png");

    private final A model;

    public BundleEntityLayer(RenderLayerParent<T, M> parent, A model) {
        super(parent);
        this.model = model;
    }
    ...
    public void render(@Nonnull PoseStack stack, @Nonnull MultiBufferSource buffer, int lighting, @Nonnull T entity,
        ...
        model.rightArm.xScale = this.getParentModel().rightArm.xScale;
        model.rightArm.yScale = this.getParentModel().rightArm.yScale;
        model.rightArm.zScale = this.getParentModel().rightArm.zScale;

        IRestrainableEntity res = (IRestrainableEntity) entity;
        if (res.getHeadRestraintId().equals(BundleRestraint.ID)) {
